...
bool less<GUID>::operator ()(const GUID & a,const GUID & b) const
{
    if(a.Data1<b.Data1)
        return true;
    if(a.Data1>b.Data1)
...
namespace guid_playback_order
{
    ...
    static const GUID repeat_track_guid = { 0x4bf4b280, 0x0bb4, 0x4dd0, { 0x8e, 0x84, 0x37, 0xc3, 0x20, 0x9c, 0x3d, 0xa2 } };
    //Random
    ...
    static const GUID shuffle_track_guid = { 0xc5cf4a57, 0x8c01, 0x480c, { 0xb3, 0x34, 0x36, 0x19, 0x64, 0x5a, 0xda, 0x8b } };
    //Shuffle (album)
    //499e0b08-c887-48c1-9cca-27377c8bfd30
    static const GUID shuffle_album_guid = { 0x499e0b08, 0xc887, 0x48c1, { 0x9c, 0xca, 0x27, 0x37, 0x7c, 0x8b, 0xfd, 0x30 } };
    //Shuffle (directory)
    //83c37600-d725-4727-b53c-bdeffe5f8dc7
    static const GUID shuffle_directory_guid = { 0x83c37600, 0xd725, 0x4727, { 0xb5, 0x3c, 0xbd, 0xef, 0xfe, 0x5f, 0x8d, 0xc7 } };
}

// {9CAB2FC1-87C9-43d5-89F6-2CF3ACD245D1}
static const GUID cfg_mode_default_guid = { 0x9cab2fc1, 0x87c9, 0x43d5, { 0x89, 0xf6, 0x2c, 0xf3, 0xac, 0xd2, 0x45, 0xd1 } };
static cfg_int8 cfg_mode_default(cfg_mode_default_guid, 0);

// {170C35DE-D789-4e15-B5BA-3C15401BF7C5}
static const GUID cfg_mode_repeat_list_guid = { 0x170c35de, 0xd789, 0x4e15, { 0xb5, 0xba, 0x3c, 0x15, 0x40, 0x1b, 0xf7, 0xc5 } };
static cfg_int8 cfg_mode_repeat_list(cfg_mode_repeat_list_guid, 0);

// {7F778F14-7FE5-44c3-8494-7867DB959FEB}
static const GUID cfg_mode_repeat_track_guid = { 0x7f778f14, 0x7fe5, 0x44c3, { 0x84, 0x94, 0x78, 0x67, 0xdb, 0x95, 0x9f, 0xeb } };
static cfg_int8 cfg_mode_repeat_track(cfg_mode_repeat_track_guid, 0);

// {9DFE3B29-D901-48b3-9FB9-8F248C9BAF13}
static const GUID cfg_mode_random_guid = { 0x9dfe3b29, 0xd901, 0x48b3, { 0x9f, 0xb9, 0x8f, 0x24, 0x8c, 0x9b, 0xaf, 0x13 } };
static cfg_int8 cfg_mode_random(cfg_mode_random_guid, 0);

// {84984D83-EB6C-43ab-BD1B-07D8CE3A232E}
static const GUID cfg_mode_shuffle_album_guid = { 0x84984d83, 0xeb6c, 0x43ab, { 0xbd, 0x1b, 0x7, 0xd8, 0xce, 0x3a, 0x23, 0x2e } };
static cfg_int8 cfg_mode_shuffle_album(cfg_mode_shuffle_album_guid, 0);

// {FE2393EB-4BC1-4661-92FB-42FF83D6B8D6}
static const GUID cfg_mode_shuffle_directory_guid = { 0xfe2393eb, 0x4bc1, 0x4661, { 0x92, 0xfb, 0x42, 0xff, 0x83, 0xd6, 0xb8, 0xd6 } };
static cfg_int8 cfg_mode_shuffle_directory(cfg_mode_shuffle_directory_guid, 0);

// {66E4C506-3FC8-4210-BF00-22C0059AC49B}
static const GUID cfg_mode_shuffle_track_guid = { 0x66e4c506, 0x3fc8, 0x4210, { 0xbf, 0x0, 0x22, 0xc0, 0x5, 0x9a, 0xc4, 0x9b } };
static cfg_int8 cfg_mode_shuffle_track(cfg_mode_shuffle_track_guid, 0);

class playlist_rg_override_callback :
    public playlist_callback_single_static
{
public:
    unsigned get_flags(){ return flag_on_playback_order_changed; }
    void on_items_added(t_size start, const pfc::list_base_const_t<metadb_handle_ptr> & p_data,const bit_array & p_selection){}//inside any of these methods, you can call playlist APIs to get exact info about what happened (but only methods that read playlist state, not those that modify it)
    void on_items_reordered(const t_size * order,t_size count){}//changes selection too; doesnt actually change set of items that are selected or item having focus, just changes their order
    ...
    virtual void on_item_focus_change(t_size p_from,t_size p_to){}//focus may be -1 when no item has focus; reminder: focus may also change on other callbacks
...
void playlist_rg_override_callback::on_playback_order_changed(t_size p_new_index)
{
    ...
    switch(p_new_index)
    {
    case 0: mode = cfg_mode_default; break;
    case 1: mode = cfg_mode_repeat_list; break;
    case 2: mode = cfg_mode_repeat_track; break;
    case 3: mode = cfg_mode_random; break;
    case 4: mode = cfg_mode_shuffle_album; break;
    case 5: mode = cfg_mode_shuffle_directory; break;
    case 6: mode = cfg_mode_shuffle_track; break;
    }
    switch(mode)
    {
    case 0: rgc.m_source_mode = rgc.source_mode_album; break;
    case 1: rgc.m_source_mode = rgc.source_mode_track; break;
    case 2: rgc.m_source_mode = rgc.source_mode_none;  break;
    }
    rgm->set_core_settings(rgc);	
}

class rg_override_page : public CDialogImpl<rg_override_page>, public preferences_page_instance
{
public:
...
const GUID rg_override_page_impl::guid = 
{ 0xb02e4a78, 0x7780, 0x43ab, { 0x9a, 0x46, 0x40, 0xe5, 0xd2, 0x74, 0x3f, 0x5b } };
...
{
	cfg_int8* cfgs[] = {
		&cfg_mode_default, &cfg_mode_repeat_list, &cfg_mode_repeat_track, &cfg_mode_random,
		&cfg_mode_shuffle_album, &cfg_mode_shuffle_directory, &cfg_mode_shuffle_track
	};
	std::copy_n(cfgs, entry_count, this->cfgs);
}

BOOL rg_override_page::on_init_dialog(CWindow, LPARAM)
{
        static_api_ptr_t<playlist_manager> pm;

		for (int i = 0; i < entry_count; ++i)
		{
			SetDlgItemTextW(IDC_D + i,
				pfc::stringcvt::string_wide_from_utf8(pm->playback_order_get_name(i)));
		}
		reset();
		return TRUE;
}

bool rg_override_page::has_changed()
{
	bool ret = false;
	for (int i = 0; i < entry_count; ++i)
		ret |= *cfgs[i] != states[i];
...
void rg_override_page::on_radio_clicked(UINT, int id, CWindow control)
{
	int idx = id - IDC_DAlbum;
	states[idx / 3] = idx % 3;
	if (has_changed()) callback->on_state_changed();
}

t_uint32 rg_override_page::get_state() {
	t_uint32 state = preferences_state::resettable;
	if (has_changed()) state |= preferences_state::changed;
	return state;
}

void rg_override_page::apply()
{
	for (int i = 0; i < entry_count; ++i)
		*cfgs[i] = states[i];
}

void rg_override_page::reset()
{
	for (int i = 0; i < entry_count; ++i)
	{
